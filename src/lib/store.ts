...
const WEEKLY_SESSIONS_KEY = "weekly_sessions";
export async function getWeeklySessions(): Promise<WeeklySession[]> {
  return load<WeeklySession[]>(WEEKLY_SESSIONS_KEY, []);
}
export async function saveWeeklySessions(sessions: WeeklySession[]): Promise<void> {
  await save(WEEKLY_SESSIONS_KEY, sessions);
}
...
// ---- Weekly Schedule Template (user-defined day → workout type mapping) ----
const WEEKLY_TEMPLATE_KEY = "weekly_template";
export async function getWeeklyTemplate(): Promise<import("./types").WeeklyTemplate | null> {
  return load<import("./types").WeeklyTemplate | null>(WEEKLY_TEMPLATE_KEY, null);
}
export async function saveWeeklyTemplate(template: import("./types").WeeklyTemplate): Promise<void> {
  await save(WEEKLY_TEMPLATE_KEY, template);
}
export async function clearWeeklyTemplate(): Promise<void> {
  await AsyncStorage.removeItem(WEEKLY_TEMPLATE_KEY);
}

// ---- Personalized exercise pool (generated by AI on plan creation) ----
const PERSONALIZED_EX_KEY = "personalized_exercises";
export async function getPersonalizedExercises(): Promise<import("./types").Exercise[]> {
  return load<import("./types").Exercise[]>(PERSONALIZED_EX_KEY, []);
}
export async function savePersonalizedExercises(exercises: import("./types").Exercise[]): Promise<void> {
  await save(PERSONALIZED_EX_KEY, exercises);
}
export async function clearPersonalizedExercises(): Promise<void> {
  await AsyncStorage.removeItem(PERSONALIZED_EX_KEY);
}
